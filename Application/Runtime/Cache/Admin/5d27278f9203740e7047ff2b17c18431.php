 ...
 <!-- Left column -->
    <div class="templatemo-flex-row">
      <div class="templatemo-sidebar">
        ...
        <nav class="templatemo-left-nav">          
          <ul>
            ...
            <li><a href="<?php echo U('Admin/Message/index');?>"  class="active"><i class="fa fa-map-marker fa-fw"></i>留言管理</a></li>
      ...
      <div class="panel panel-default">
         <!--标题-->
       <?php if(is_array($meresult)): $i = 0; $__LIST__ = $meresult;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="panel-body">
            <p class="text-primary" style="font:14px '微软雅黑';"><?php echo ($vo["username"]); ?>于<?php echo ($vo["publishtime"]); ?>发表：
              <span class="badge" data-toggle="modal" data-target="#myModal"  style="float:right;">
                   回复
              </span>
            </p>
            <span class="text-info" style="font:14px '微软雅黑';"><?php echo ($vo["messagecontent"]); ?></span>
         </div><?php endforeach; endif; else: echo "" ;endif; ?>
         <!--标题-->
         <!--回复-->     
        <?php if(is_array($result)): $i = 0; $__LIST__ = $result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo["rname"] != null): ?><div class="panel-footer">
             <p class="text-info" style="font:14px '微软雅黑';"><?php echo ($vo["rname"]); ?>于<?php echo ($vo["republishtime"]); ?>回复：</p>   
             <span class="text-success" style="font:14px '微软雅黑';"><?php echo ($vo["mcontent"]); ?></span>
         </div> 
     <?php else: ?>
         <div class="panel-footer">
             <p class="text-info" style="font:14px '微软雅黑';"></p>   
             <span class="text-success" style="font:14px '微软雅黑';">暂无回复</span>
         </div><?php endif; endforeach; endif; else: echo "" ;endif; ?>
         <!--回复-->
      </div>
          <!----内容区--->
        </div>
          <footer class="text-right" style="margin-top:100px;">
      ...
      <div class="modal-body">
        <!------->  
         <div class="form-group">
           ...
           <input type="hidden" value="<?php echo ($mid); ?>" id="rid" name="rid"/>
         </div> 
        <!------->
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
        <button type="button" class="btn btn-primary" onclick="hf();">回复</button>
      </div>
     </form>
    </div>
  </div>
...
<script type="text/javascript">
   ...
   function hf(){
	   ...
	   var ajaxSidUrl='<?php echo U("Message/returnmessage");?>'; 
	   $.ajax({
            //提交数据的类型 POST GET
          type: "POST",
           //提交的网址
