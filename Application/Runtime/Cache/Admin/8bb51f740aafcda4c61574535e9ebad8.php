      ...
      <!-- **放入管理者名字** -->
        ...
        <nav class="templatemo-left-nav">          
          <ul>
            ...
            <li><a href="<?php echo U('Admin/Filemannger/files');?>"  class="active"><i class="fa fa-database fa-fw"></i>博客管理</a></li>
      ...
      <div class="templatemo-content col-1 light-gray-bg">
        ...
        <div class="templatemo-content-container">         
          <div class="templatemo-content-widget white-bg">
            <h2 class="margin-bottom-10">博客管理</h2>
            <div class="panel panel-default no-border">
              <div class="panel-heading border-radius-10">
                <h2>
                <!---放入博客类型--->
              <a href="<?php echo U('Filemannger/files');?>?id=" style="font-size:14px;">
               <i class='icon-list-ul 
                 <?php if(($vo["id"]) == $_SESSION['cs']): ?>text-info<?php endif; ?>'>
               </i> 所有
              </a>
            <?php if(is_array($filtype)): foreach($filtype as $key=>$vo): ?><a href="<?php echo U('Filemannger/files');?>?id=<?php echo ($vo["id"]); ?>" style="font-size:14px;">
               <i class='icon-list-ul 
                 <?php if(($vo["id"]) == $_SESSION['files']): ?>text-info<?php endif; ?>'>
               </i> <?php echo ($vo["title"]); ?>
            ...
            <?php if(is_array($data)): foreach($data as $key=>$vo): ?><tr>
                ...
                <td>
                  <button type="button" class="btn btn-primary" onclick="window.location.href='<?php echo U('Filemannger/delfile','','');?>/id/<?php echo ($vo["id"]); ?>'">删除</button>
                  <button type="button" class="btn btn-primary" onclick="window.location.href='<?php echo U('Filemannger/update','','');?>/id/<?php echo ($vo["id"]); ?>'">更新</button>
                  <button type="button" class="btn btn-primary" onclick="window.location.href='<?php echo U('Filemannger/fpinglun','','');?>/id/<?php echo ($vo["id"]); ?>'">评论</button>
                </td>
